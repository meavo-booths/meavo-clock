...
// TLS: when API_BASE_URL is https, validate the server certificate against
// the pinned root CA below (default). Only set API_INSECURE_TLS to 1 for
// bench testing against a host whose chain doesn't terminate at API_ROOT_CA —
// insecure mode lets an attacker on the WiFi capture DEVICE_API_KEY.
#define API_INSECURE_TLS 0

// ISRG Root X1 — root CA for Let's Encrypt certs on clock.meavo.app
// (valid until 2035-06-04; re-pin if the host's CA changes).
#define API_ROOT_CA \
  "-----BEGIN CERTIFICATE-----\n" \
